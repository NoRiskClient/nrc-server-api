...
public record ServerAPI(CoreAPI coreAPI, JavaPlugin plugin) implements NoRiskServerAPI {

  public static ServerAPI create(Plugin plugin) {
    try {
      if (Spigot.getInstance() == null) {
        plugin.getLogger().severe(
            "NoRiskClient-Server-API Spigot module is not loaded! Make sure it's in your plugins folder.");
        return null;
      }

      if (Spigot.getCoreApi() == null) {
        plugin.getLogger()
            .severe("NoRiskClient-Server-API Spigot module failed to initialize properly.");
        return null;
      }
  ...
  public void sendPacket(UUID uuid, OutPayload packet) {
    ...
    BukkitRunnable runnable = new BukkitRunnable() {
      @Override
      public void run() {
        player.sendPluginMessage(plugin, coreAPI.getPluginChannel(),
